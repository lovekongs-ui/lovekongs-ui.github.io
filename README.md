...
2026-09-22에 GitHub 공식 안내를 확인했습니다. 저장소 생성·업로드·배포는 사용자가 직접 진행합니다.
...
- **하위 경로**: 현재 사이트는 내부 `#앵커`와 내장 자산만 사용해 저장소 하위 경로에서도 동작합니다. 향후 파일을 분리하면 `./style.css`처럼 상대 경로를 사용하고 `/style.css`처럼 도메인 루트 기준 경로는 피합니다.
...
- 첫 화면의 CSS 도형은 시각적 장식입니다. 프로젝트 목록은 텍스트 중심으로 구성했고, AGAMI에는 실제 서비스 시연 영상을 사용했습니다.
...
- 모션을 끄면 도형의 포인터·스크롤 모션, 진입 애니메이션, 호버 이동과 부드러운 스크롤을 멈춥니다. 읽기 진행선과 상세 열림 아이콘은 상태 표시로 즉시 갱신됩니다.
- JavaScript를 꺼도 내용·링크·상세 펼침은 사용할 수 있습니다.

## 검증 기록

세부 결과와 한계는 `qa/VERIFICATION.md`를 참고하세요. 사이트 자체는 더블클릭으로 열어도 되며, 검증에는 로컬 HTTP 서버를 사용했습니다. 배포 후 실제 Pages 환경과 휴대전화에서도 최종 확인하세요.
